...
export default function DestinationsManager() {
  ...
  const handleDeleteRoom = async (_new: INew) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          setLoading(true);
          const rs = await destinationService.deleteNew(_new.id);
  ...
  const handleClickRow = (record: INew) => {
    navigate(DEFINE_ROUTERS_ADMIN.destinationDetail.replace(":id", record.id));
  };
      ...
      <div className="flex flex-col justify-start items-start space-y-5 w-full">
        ...
        <div className="flex flex-row justify-between items-center w-full">
          ...
          <Button
            ...
            variant="filled"
            onClick={() => {
              navigate(DEFINE_ROUTERS_ADMIN.createDestination);
            }}
          >
            Thêm 1 bài viết mới
          </Button>
